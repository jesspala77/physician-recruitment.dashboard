...
def test_train_model_saves_model(tmp_path: Path):
    csv_path = create_training_csv(tmp_path)
    model_path = tmp_path / 'trained_model.pkl'

    metrics = train_model(str(csv_path), str(model_path), verbose=False)
