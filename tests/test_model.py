...
def create_sample_training_data(tmp_path: Path) -> Path:
    ...
    data = {
        'physician_specialty': ['ENT', 'ENT', 'Allergy', 'Allergy', 'Cardiology', 'Cardiology', 'Oncology', 'Oncology', 'ENT', 'Allergy'],
...
def test_train_model_and_load_predictor(tmp_path: Path):
    data_path = create_sample_training_data(tmp_path)
    model_path = tmp_path / 'model.pkl'

    result = train_model(str(data_path), str(model_path), verbose=False)
    ...
    test_input = {
        ...
        'historical_enrollment': 0.4,
        'site_experience': 3,
        'visit_burden': 8,
        'eligibility_strictness': 2,
        'specialty_match': 1.0,
    ...
    prediction_label, confidence = predictor.predict(test_input)
    assert prediction_label in {'Strong Match', 'Weak Match'}
